...
def ip_data(ip_add):
    """This function takes in a list of tuples of ip addresses and returns a
     dictionary of unique ip address pairs as the key and the number of occurrences
     as the value."""
    unique_tup = []
    num_list = []
...
def create_kml(data_b, dst_dict: dict):
    """Function that takes a database file and an IP destination dictionary and
    creates a KML file locating the destination IP addresses."""
    try:
...
def locateip(db_file, ip_add):
    """This is the main function. It sends ip addresses to the ip_data function for
    processing. A dictionary is returned and the results formatted in a table.
    Finally a kml file is created from the destination ip addresses."""
    try:
        # Process the ip data.
