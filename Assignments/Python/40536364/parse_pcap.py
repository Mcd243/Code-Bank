...
def parser(pcap_file):
    """Parser function. Extracts the relevant data from the packets.
    TCP data has an extended data capture for later processing.
    Packet types are dynamically assigned to a dictionary with the
    relevant data to form a summary table for the packet capture file."""
    ...
    protocol_list = ["HOPOPT", "ICMP", "IGMP", "GGP", "IPv4", "ST", "TCP", "CBT", "EGP",
                     "IGP", "BBN-RCC-MON", "NVP-II",
                     "PUP", "ARGUS", "EMCON", "XNET", "CHAOS", "UDP", "MUX", "DCN-MEANS",
                     "HMP", "PRM", "XNS-IDP",
                     "TRUNK-1", "TRUNK-2", "LEAF-1", "LEAF-2", "RDP", "IRTP", "ISO-TP4",
                     "NETBLT", "MFE-NSP",
                     "MERIT-INP", "DCCP", "3PC", "IDPR", "XTP", "DDP", "IDPR-CMTP", "TP++",
                     "IL", "IPv6", "SDRP",
                     "IPv6Route", "IPv6-Frag", "IDRP", "RSVP", "GRE", "DSR", "BNA", "ESP",
                     "AH", "I-NLSP", "SWIPE",
                     "NARP", "MOBILE", "TLSP", "SKIP", "IPv6-ICMP", "IPv6-NoNxt", "IPv6Opts",
                     "", "CFTP", "",
                     "SAT-EXPAK", "KRYPTOLAN", "RVD", "IPPC", "", "SAT-MON", "VISA", "IPCV",
                     "CPNX", "CPHB", "WSN",
                     "PVP", "BR-SAT-MON", "SUN-ND", "WB_MON", "WB-EXPAK", "ISO_IP", "VMTP",
                     "SECURE-VMTP", "VINES",
                     "TTP", "IPTM", "NSFNET-IGP", "DGP", "TCF", "EIGRP", "OSPFIGP",
                     "Sprite_RPC", "LARP", "MTP",
                     "AX.25", "IPIP", "MICP", "SCC_SP", "ETHERIP", "ENCAP", "", "GMTP",
                     "IFMP", "PNNI", "PIM", "ARIS",
                     "SCPS", "QNX", "A/N", "IPComp", "SNP", "Compaq-Peer", "IPX_in-IP",
                     "VRRP", "PGM", "", "L2TP",
                     "DDX", "IATP", "STP", "SRP", "UTI", "SMP", "SM", "PTP", "ISIS over IPv4",
                     "FIRE", "CRTP", "CRUDP",
                     "SSCOPMCE", "IPLT", "SPS", "PIPE", "SCTP", "FC", "RSVP_E2E_IGNORE",
                     "Mobility Header", "UDPLite",
    ...
    # https: // jon.oberheide.org / blog / 2008 / 10 / 15 / dpkt - tutorial - 2 -
    # parsing - a - pcap - file /
    # Jon Oberheide
    ...
    for time_s, buf in pcap:
        ...
        ip_add.append((socket.inet_ntoa(i_p.src), socket.inet_ntoa(i_p.dst)))
        if i_p.p == 6:
            info = [time_s, len(buf)]
            key = proto_dict[i_p.p]
            summary_data[key].append(info)
            tcp_data.append(tcp)
        # https://stackoverflow.com/questions/20585920/how-to-add-multiple-values-to-a-
        # dictionary-key-in-python
        # President James K. Polk
        # Dec 14 '13 at 17:08
        # If the ip protocol type is not in proto_dict then create a new key value pair
        # in the summary_data dictionary
        # and append the info to the value list
        elif i_p.p in proto_dict.keys():
            if proto_dict[i_p.p] not in summary_data:
                info = [time_s, len(buf)]
                key = proto_dict[i_p.p]
    ...
    open_file.close()
    return tcp_data, tcp_data, ip_add, timestamp, summary_data


def pcap_parser(pcap_file):
    """ Pass the pcap file to the parser function and collect the relevant data.
    Compile the data into various data objects to be used in further functions.
    The results are returned in a dictionary for later processing."""
    ...
    try:
        ...
        print(f"!! There was a problem opening {pcap_file}({err.__class__.__name__}) : "
