    ...
    try:
        ...
        if choice == "x":
            ...
            if int(choice) in file_dict:
                ...
                FLAG = False
                # Pass the pcap file to the function and store the relevant data in a dictionary
                # that references the data from the parser to their relevant modules.
                parsed_data_dict = pcap_parser(pcap_file)
                # print(packet_data)
            else:
                print("The number you have entered is not in the list.")
    except FileNotFoundError as err:
        print(f"!! There was a problem opening {pcap_file}({err.__class__.__name__}) : {err}",
              file=sys.stderr)
...
input(f"""Press enter to see unique email addresses in the From: and To: fields and image  
uri's from http headers from {pcap_file}...""")
try:
