...
def mean_p_len(data_list: list) -> float:
...
def ts_convert(timestamp: float) -> str:
...
def summary_table(summary_data: list) -> PrettyTable:
    ...
    try:
        ...
        for key in summary_data:
            summary.add_row(
                [key, len(summary_data[key]), ts_convert(summary_data[key][0][0]), ts_convert(summary_data[key][-1][0]),
