...
def summary_table(tcp_list, udp_list, igmp_list, other_list):
    ...
    if len(tcp_list) > 0:
        summary.add_row(
            ["TCP", len(tcp_list), ts_convert(tcp_list[0][0]), ts_convert(tcp_list[-1][0]), mean_p_len(tcp_list)])
    if len(udp_list) > 0:
        summary.add_row(
            ["UDP", len(udp_list), ts_convert(udp_list[0][0]), ts_convert(udp_list[-1][0]), mean_p_len(udp_list)])
    if len(igmp_list) > 0:
        summary.add_row(
            ["IGMP", len(igmp_list), ts_convert(igmp_list[0][0]), ts_convert(igmp_list[-1][0]), mean_p_len(igmp_list)])
    if len(other_list) > 0:
        summary.add_row(["OTHER", len(other_list), ts_convert(other_list[0][0]), ts_convert(other_list[-1][0]),
                         mean_p_len(other_list)])
    return summary
...
def find_email(data):
    ...
    try:
        match = re.findall(
            r"From: \"[a-zA-Z]+ [a-zA-Z]+\" <\w+@\w+\.\w+>|FROM: <\w+@\w+\.\w+>|TO: <\w+@\w+\.\w+>|To: <\w+@\w+\.\w+>",
            data)
        return match
...
def create_kml(dst_dict: dict) -> None:
    try:
...
def main(uri_data: list,
    ...
    # Create a kml file.
    create_kml(dst_dict)
    # Create a line graph with some data
    pcap_stats(timestamp)
