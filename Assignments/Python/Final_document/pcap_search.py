...
def find_email(data):
    ...
    match = re.findall(
        r"(?i)from:\s?\"[a-zA-Z]+ [a-zA-Z]+\" <\w+@\w+\.\w+>|"
        r"from:\s?<\w+@\w+\.\w+>|TO:\s?<\w+@\w+\.\w+>|to:\s?\w+@\w+\.\w+", data)
    return match


def find_emails(tcp_data: list):
    """Finds email addresses in tcp data and returns a set of unique
    email-addresses form the From: field and returns a set of unique
    email-addresses form the To: field"""
...
def find_image(data):
    ...
    try:
        match = re.findall(r"(?i)[-\/\w.]+[.]gif|[-\/\w.]+[.]jpg|[-\/\w.]+[.]png", data)
        return match
...
def search(packet_data):
    """Extracts unique email addresses in the From: and To: fields
    ...
    try:
        ...
        input("\nPress enter to display all the unique email addresses in the pcap"
              " file in the From: field.")
        for email in from_emails:
            print(f"[*]    From: {email}")
        input("\nPress enter to display all the unique email addresses in the pcap "
              "file in the To: field.")
