...
def ts_convert(timestamp) -> str:
    ...
    resolved_dt: str = f"{d_t:%H:%M:%S}"
    return resolved_dt


def pcap_stats(timestamp):
    """A function to group timestamps into time periods and count
    ...
    first_timestamp = []
    hr_timestamp = []
    time_grouping = int(input("Enter a time grouping number to display "
                              "plot the graph. 10 is recommended."))
    first = timestamp[0]
